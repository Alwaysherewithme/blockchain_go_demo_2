...
/*
	go func() {
		...
		// for i := 0; i < 2**32; i++ {
		// 	if sha256(sha256(block_header)) < 0.001 {
		// 		flag = true
		// 		break
		// 	} else {
		// 		continue
		...
		for i := 0; i < len(cache); i++ {
			// if sha256.Sum256([]byte(cache[i].Value)) < ([32]byte("10")) {
				// sha256.Sum256([]byte(fmt.Sprintf("%d%s%d%s", b.Index, b.PreviousHash, b.Timestamp, b.Data)))
				block := generateNextBlock(cache[i].Data)
